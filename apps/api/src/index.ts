...
async function generateGeminiText(prompt: string, system: string): Promise<string> {
...
async function streamGemini(
  ...
  try {
    ...
    // 429 or 503 fallback to Groq
    if (err.status === 429 || err.status === 503 || err?.toString()?.includes("503")) {
...
app.post("/api/v1/name-chat", async (req, res) => {
  try {
    ...
    if (!message?.trim()) return res.status(400).json({ title: "New Chat" });
